...
{
  unsigned char * sig, * key, * ahost, * sigkey;
  size_t sig_len, ahost_len, sigkey_len;

  key = get_xkey(key_len, xhost, xhost_len);
  sig = get_xsig(&sig_len, xhost, xhost_len);
  sigkey = get_sigkey(&sigkey_len);

  // We only provide the service for the single key of attacker's choice
  //ahost = get_xhost(&ahost_len, side);

  // if((ahost_len != xhost_len) || memcmp(xhost, ahost, xhost_len))
