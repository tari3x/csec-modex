...
let simplify_lets p =
  let rec simplify defs = function
    | Let (VPat v, Var v') :: p -> simplify defs (Iml.subst [v] [Var v'] p)
    | Let (VPat v, e) :: p when Iml.refcount v p = 0 -> simplify defs p
    | Let (VPat v, e) :: p ->
      begin match Option.try_with (fun () -> List.assoc e defs) with
      | Some v' ->
        simplify defs (Iml.subst [v] [Var v'] p)
      | None ->
  ...
  let rec normalize p =
    ...
    *)
    let rec explicate_lens args = function
      ...
      | e :: es ->
        ...
        | Some width ->
          ...
          match Option.try_with (fun () -> E.typecheck (Bs_int itype) e) with
          | None -> e :: explicate_lens args es
          | Some () ->
    ...
    and extract_fun = function
      | Sym (Fun _, _) as e -> E.descend extract_fun e
      | e -> mk_var (extract e)
    ...
    match p with
      ...
      | In vs as s :: p ->
        ...
        s :: normalize p
      | New (v, t) as s :: p ->
        S.add_fact (S.in_type (Var v) t);
        s :: normalize p
      | (Test _ | Test_eq _ | Out _ | Event _ | Yield | Comment _ ) as s :: p ->
...
let assert_equal ~expected ~actual =
  if expected <> actual
  then fail "Expected: \n%s\ngot \n%s\n" (Iml.to_string expected) (Iml.to_string actual)
...
let test_normal_form_concat () =
  ...
  let e = Concat [String "msg"; l1; BS (Int 20L, itype);
  ...
  *)
  ...
  let extract : exp -> exp = function
    | Concat es as e ->
      ...
      then warn "Concat not injective: %s" (show_fun f_c def);
...
let compute_parsing_rules fun_types (parsers: sym_defs) (concats: sym_defs): parsing_eq list =
  ...
  Sym.Map.iter check_parser_is_matched parsers;
  !parsing_eqs
...
let check_concat_safety facts e c c_def =
  ...
  let result = is_injective_concat c c_def && inrange facts e c c_def in
