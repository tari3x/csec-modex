...
module Template: sig
  ...
  val var_types : t -> Typing.type_ctx
  val fun_types : t -> Typing.fun_type_ctx

  val defined_functions : t -> Sym.t list
  ...
  let collect_types env q =
    ...
    Sym.Map.of_list (List.map (fun (f, (ts, t)) ->
      (Sym.T.Fun (f, List.length ts), (ts, t))) !fun_types)
  ...
  let read_file ~cv_lib_name ~name =
    ...
    let rec split_template: string list ref -> string list -> unit = fun dest -> function
      ...
      | l1 :: l2 :: ls' when trim l2 = "<Type hints>" ->
...
let write_cv model =
  ...
  let print_fun_def is_injective sym def =
    if List.mem sym (Template.defined_functions model.template)
    then
...
let main () =
  ...
  *************************)
  ...
  let client = Typing.check defs fun_types template_var_types [] client in
  let server = Typing.check defs fun_types template_var_types [] server in
