...
let unify_up_to_x next_stage t1 t2 =
  ...
  try 
	...
	NoLink -> true
      | TLink t -> 
	  begin
	    let set = List.assq f (!Param.secret_vars_with_sets) in
	    match set with
	      None ->
...
let check_testunif_true next_stage hypbefore hypafter concl hist constra l1 l2 =
  if (List.for_all check_any_x hypbefore)
      ...
      && (List.for_all (function
          FunApp(f2,[]) -> List.memq f2 (!Param.secret_vars)
	| _ -> false) l1) then
    begin
      (*
     dec_out_rule (fun hypbefore hypafter concl hist constra ->
       Display.display_rule (List.rev_append hypbefore hypafter, concl, hist, constra)) hypbefore hypafter concl hist constra l1 l2;
      *)
     (* Testunif true *)
...
let inst_elim next_stage repeat_next_stage hypbefore hypafter concl hist constra l1 l2 =
   ...
   List.iter2 (fun l1 l2 -> 
	 ...
	 when (contains_att_x v1)
         (* && not ((List.memq f (!Param.secret_vars)) ||
                    f.f_cat = General_var *)
       ->
	 if List.exists do_instantiate (!Param.secret_vars_with_sets) then
	   begin
...
let simplify next_stage repeat_next_stage ((hyp, concl, hist, constra) as r) = 
  if (not (!Param.non_interference)) || (detect_atteq r) then
    next_stage r
  else
...
let selfun ((hyp, concl, _, _) as r) = 
  if not ((!Param.non_interference) && (is_bad concl) && (hyp != [])) then -1 else
