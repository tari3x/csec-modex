  ...
  let rec sel (nold, sold) n = 
    function [] -> 
	  ...
	  (* Guarantee that p(x) is never selected when we decompose data
         ...
         sel (nold, sold) (n+1) l
      | (Pred(p,lp) as h::l) -> 
	  if find_same_format (p,lp) (!no_unif_set) < 0 then
	    if sold < -1 
 	    then sel (n,-1) (n+1) l 
            else sel (nold, sold) (n+1) l
	  else
	    begin
    ...
    function [] -> 
	  ...
	  (* Guarantee that p(x) is never selected when we decompose data
	     ...
	     the decomposition of data constructors. *)
            sel (nold, sold) (n+1) l
      | (Pred(p,lp) as h::l) -> 
	    let stmp = find_same_format (p,lp) (!no_unif_set) in
            let snew = 
	      if stmp < 0
	      then stmp 
	      else fact_size h 
...
let selection_fun_weight ((hyp, concl, _, _) as rule) =
  let wconcl = 
    if List.exists (fun h -> matchafactstrict h concl) hyp then -2 else 0
  in
